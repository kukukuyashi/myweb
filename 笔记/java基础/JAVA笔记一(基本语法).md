...
第一个 JAVA 程序，创建文件 **HelloWorld.java(文件名需与类名一致)**, 代码如下：

```java
package com.itheima.test;
// package: 表示当前的类定义在那个包下
// 这段代码不用自己写，是IDEA自动生成的

public class HelloWorld2 {
    // class: 表示一个类
    // HelloWorld: 类的名字
    // public: 表示当前类是公开的，可以被其他类访问
    // {}：表示类的范围，类的开始和结束
...
Java语言提供了**八**种基本类型。六种数字类型（四个整数型，两个浮点型），一种字符类型，还有一种布尔型。
...
1. **导包（这一步可以忽略idea自动生成）**：确定Scanner的位置
...
public class VariableDemo6_qiuhe {
    static void main(String[] args) {
        // 定义两个整数类型的变量num1和num2，键盘录入数据分别为两个变量赋值。
        // 求两个数的和并进行打印。
        
        // 创建Scanner对象,获取键盘录入数据
        Scanner sc = new Scanner(System.in);
        // 获取键盘录入数据
        int num1 = sc.nextInt();
        int num2 = sc.nextInt();
...
当+操作中出现字符串时，此时就是字符串的连接符，会将前后的数据进行拼接，并产生一个**新的字符串**
...
当+操作中出现了字符，会拿着字符到计算机内置的**ASCII码表中去查对应的数字**，然后再进行计算。
...
如果用户名输入正确了，那么我们再判断密码是否正确，是符合业务逻辑的。但是如果用户名输入错误了，那么现在还有必要去比较密码吗？没有不要了。
